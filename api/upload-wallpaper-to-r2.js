...
export default async function handler(req, res) {
  ...
  try {
    ...
    for await (const chunk of req) {
      ...
      if (totalSize > MAX_WALLPAPER_BYTES) {
        return res.status(400).json({ error: "File wallpaper kegedean, maksimal 5MB" });
      }
      chunks.push(chunk);
    }
    ...
    const key = `wallpapers/${Date.now()}-${fileName}`;
    ...
    return res.status(500).json({ error: "Upload wallpaper gagal", detail: err.message });
  }
}
